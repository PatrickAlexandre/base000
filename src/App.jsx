...
function QuestJournal({playerClass,multiplier}){
  return <div className="quest-journal">
    {playerClass?<article className="quest-entry class-quest" style={{'--class-color':playerClass.color}}><div className="quest-entry-head"><span className="quest-entry-icon"><FontAwesomeIcon icon={playerClass.icon}/></span><div><small>QUÊTE PRINCIPALE DE CLASSE</small><strong>{playerClass.type} · {playerClass.name}</strong></div></div><p>{playerClass.mission}</p><span className="quest-reward">Multiplicateur actuel des récompenses : × {multiplier.toFixed(2)}</span></article>:<article className="quest-entry"><small>JOURNAL</small><p>Aucune mission de classe active. Tu peux choisir une classe depuis ton profil pour débloquer une ligne narrative adaptée.</p></article>}
    <article className="quest-entry"><small>À VENIR</small><p>Les quêtes secondaires, objectifs quotidiens et ramifications de l’ARG apparaîtront ici.</p></article>
  </div>
}
