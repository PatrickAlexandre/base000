...
const MBTI_CLASSES = [
  ['INTJ', 'Architecte', 'Bâtir une vision et transformer le chaos en système.'],
  ['INTP', 'Alchimiste', 'Explorer les idées, comprendre les mécanismes et découvrir des solutions inattendues.'],
  ['ENTJ', 'Commandant', 'Prendre la tête, organiser les forces et mener des objectifs ambitieux.'],
  ['ENTP', 'Inventeur', 'Défier les évidences, expérimenter et ouvrir de nouvelles voies.'],
  ['INFJ', 'Oracle', 'Lire les dynamiques profondes et guider les autres vers une vision porteuse de sens.'],
  ['INFP', 'Gardien des valeurs', 'Défendre ce qui compte et donner une direction humaine aux choix.'],
  ['ENFJ', 'Mentor', 'Faire grandir les autres, fédérer et transformer une vision en mouvement collectif.'],
  ['ENFP', 'Éclaireur', 'Créer des connexions, découvrir des opportunités et insuffler de l’élan.'],
  ['ISTJ', 'Sentinelle', 'Maintenir l’ordre, fiabiliser les systèmes et accomplir les missions avec rigueur.'],
  ['ISFJ', 'Protecteur', 'Préserver, soutenir et veiller au bon fonctionnement du groupe.'],
  ['ESTJ', 'Capitaine', 'Structurer l’action, décider vite et faire avancer les opérations.'],
  ['ESFJ', 'Intendant', 'Créer de la cohésion, prendre soin du collectif et maintenir les liens.'],
  ['ISTP', 'Artisan', 'Résoudre les problèmes concrets avec précision, calme et autonomie.'],
  ['ISFP', 'Rôdeur', 'Suivre son instinct, agir avec sensibilité et s’adapter librement au terrain.'],
  ['ESTP', 'Avant-garde', 'Agir dans l’instant, prendre des risques mesurés et saisir les opportunités.'],
  ['ESFP', 'Barde', 'Créer de l’énergie, vivre l’expérience à fond et entraîner les autres dans l’aventure.']
]
...
function classMeta(code) { const item = MBTI_CLASSES.find(([mbti]) => mbti === code); return item ? { code: item[0], title: item[1], mission: item[2] } : null }
...
function ProfileForm({ initialProfile, plan, onSave, onCancel }) {
  ...
  const [form, setForm] = useState({ firstName: initialProfile?.firstName || '', lastName: initialProfile?.lastName || '', birthDate: initialProfile?.birthDate || '', sex: initialProfile?.sex || '', countryCode: initialProfile?.countryCode || '', classCode: initialProfile?.classCode || '', photo: null })
  ...
  const selectedClass = classMeta(form.classCode)
  ...
  async function submit(event) {
    ...
    const base = { ...initialProfile, firstName: form.firstName.trim(), lastName: form.lastName.trim(), classCode: form.classCode || '', photo, birthDate: canEditFixed ? form.birthDate : initialProfile.birthDate, sex: canEditFixed ? form.sex : initialProfile.sex, countryCode: canEditFixed ? form.countryCode : initialProfile.countryCode, countryName: canEditFixed ? (country?.name || initialProfile?.countryName || '') : initialProfile.countryName }
